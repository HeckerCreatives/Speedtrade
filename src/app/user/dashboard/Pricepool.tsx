...
export default function Pricepool() {
  ...
  return (
    <>
    {qualify === true && (
         <div className=' relative w-full max-w-[1440px] h-auto flex items-center justify-center my-20 md:my-16'
         style={{
             width: '100%',
         ...
         >
             ...
             {showCoins === true ? (
                 <img
                 src='/Piggy-bank.gif' // Replace with a static image when not playing
                 width={130}
             ...
             <div className=' absolute right-2 md:relative z-20 w-full max-w-[70%] h-[70px] rounded-md bg-slate-900 flex items-center justify-center'>
                 <p className=' absolute top-2 left-2 text-xs text-slate-400'>Prize Pool</p>
                 <p className=' text-sm md:text-xl font-medium '>Php {amount.toLocaleString()} <span className=' text-green-500'>/</span> Php {pool?.pricepool.toLocaleString()}</p>
             </div>
