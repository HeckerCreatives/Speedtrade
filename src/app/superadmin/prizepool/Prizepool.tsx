...
const getName = (miner: any) => {
    if(miner === 'quick_miner'){
        return 'Quick Miner'
...
const getBenificiary = (data: string) => {
  ...
  const formattedMiners = trimmedMiners.map((miner) => getName(miner));
...
useEffect(() => {
  ...
  return (
    <div className=' w-full flex flex-col'>
        ...
        <div className=' w-fit flex flex-col gap-8 bg-slate-950 p-6'>
            <div className=' w-fit flex flex-wrap gap-4'>
                <div className=' bg-slate-800 w-[300px] p-6 text-sm flex flex-col gap-2'>
                    <p>Current Value</p>
                    <input type="number" value={currentvalue} onChange={(e) => setCurrentValue(e.target.valueAsNumber)} className='text-sm p-2 w-full max-w-[300px]  rounded-md bg-slate-600'/>

                </div>

                <div className=' bg-slate-800 p-6 w-[300px] text-sm flex flex-col gap-2'>
                    <p>Price Pool</p>
                    <input type="number" value={pricepool} onChange={(e) => setPricepool(e.target.valueAsNumber)} className='text-sm p-2 w-full max-w-[300px]  rounded-md bg-slate-600'/>
            ...
            {options.map((option) => (
               ...
               <p className=' text-xs'>{getName(option)}</p>
           </div>
            ))}
            ...
            <div className=' flex items-center gap-4 text-sm'>
                <button disabled={loading2} onClick={distributePricepool} className='bg-slate-700 px-4 py-2 rounded-md flex items-center gap-2'>
                {loading2 === true && (
                    <div className=' spinner'></div>
                  )}
                  Distribute</button>
                <button disabled={loading2} onClick={updatePricepool} className='bg-gradient px-4 py-2 rounded-md flex items-center gap-2'>
                  {loading2 === true && (
                    <div className=' spinner'></div>
                  )}
                  Save</button>
        ...
        <div className=' w-full flex flex-col mt-12 p-4 bg-slate-800'>
