'use client'
import { changepasswordsuperadmin, ChangePasswordSuperadmin } from '@/app/validation/schema'
...
export default function Changepassword() {
  ...
  const changePassword = async (data: ChangePasswordSuperadmin) => {
    setLoading(true)
    try {
         const request = axios.post(`${process.env.NEXT_PUBLIC_URL}/staffusers/changepassadmin`,{
        ...
        <div className=' flex flex-col max-w-[500px] w-full h-auto bg-slate-800 rounded-sm p-6 shadow-lg'>
            ...
            <form onSubmit={handleSubmit(changePassword)} className=' w-full flex flex-col gap-1  text-xs mt-6'>
                ...
                <div className=' relative w-full'>
                    <input type={shownew} placeholder='New Password' className=' p-2 text-xs rounded-sm w-full text-black' {...register('newpasswordsuperadmin')} />
                    {shownew === 'password' ? 
                    (
                ...
                <div className=' relative w-full'>
                    <input type={showconfirm} placeholder='Confirm Password' className=' p-2 text-xs rounded-sm w-full text-black' {...register('confirmpasswordsuperadmin')}/>
                ...
                <button className=' px-6 py-2 text-sm font-semibold rounded-sm bg-gradient w-fit mt-6 flex items-center justify-center gap-2'>
                    {loading === true && (
                    <Spinner/>
                    )}
                    Save</button>
